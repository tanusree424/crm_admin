...
@endsection
							...
							<div class="page-header d-xl-flex d-block">
								<div class="page-leftheader">
									<h4 class="page-title"><span class="font-weight-normal text-muted ms-2">{{lang('Create Stn', 'menu')}}</span></h4>
								</div>
							</div>
							<!--End Page header-->
							...
							<div class="col-xl-12 col-lg-12 col-md-12">
								<div class="card ">
									<div class="card-header border-0 d-md-max-block">
										<h4 class="card-title mb-md-max-2">{{lang('Create Stn', 'menu')}}</h4>
						...
						<select class="form-select select2  form-select" name="spare_code" id="spare_code">
							<option value="">Select Spare Code</option>
							@foreach($inventryList as $material)
								<option value="{{ $material->material_code }}"  data-source-location-id="{{ $material->location_id }}" data-inventry-id="{{ $material->inventry_id }}" data-quantity="{{ $material->quantity }}" data-material-id="{{ $material->material_id }}" data-material-code="{{ $material->material_code }}" data-material-name="{{ $material->material_name }}" data-material-description="{{ $material->spare_description }}">{{ $material->material_code }}</option>
							@endforeach
						</select>
                                </div>
                        </div>
                        <div class="col-sm-6 col-md-3">
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control " name="spare_name" value="" autocomplete="off" disabled>
                                                            </div>
                        </div>
                        <div class="col-sm-6 col-md-3">
                            <div class="form-group">
                                <label class="form-label">Description</label>
                                <input type="text" class="form-control" name="spare_description" disabled>
                            </div>
                        </div>
                        ...
                        <div class="col-sm-6 col-md-3">
                            <div class="form-group">
                                ...
                                <input type="number" class="form-control " name="quantity" id="quantity" value="" autocomplete="off" disabled>
                            </div>
                        </div>
                        ...
                        <div class="col-sm-6 col-md-3">
                            <div class="form-group">
                                ...
                                <input type="number" class="form-control " name="transfer_quantity" id="transfer_quantity" value="" autocomplete="off">
                            </div>
                        </div>
        ...
        <script type="text/javascript">
			...
			(function($)  {
				...
				$("#spare_code").on('change', function() {
					...
					let materialId = selectedOption.data('material-id');
					let materialCode = selectedOption.data('material-code');
					let materialName = selectedOption.data('material-name');
					let materialDescription = selectedOption.data('material-description');
					...
					$('input[name="spare_description"]').val(materialDescription);
				...
				$('#inventory_form').on('click', function (e) {
                    ...
                    formData.append('inventry_id', $('#spare_code').find('option:selected').data('inventry-id'));
                    formData.append('source_location_id', $('#spare_code').find('option:selected').data('source-location-id'));
					...
					formData.append('destination_location_id', $('select[name="location_id"]').val());
					formData.append('quantity', $('input[name="transfer_quantity"]').val());

					formData.append('_token', $('meta[name="csrf-token"]').attr('content'));
					e.preventDefault();
					$.ajax({
						type:'POST',
						url: SITEURL + "/admin/stn/create",
						data: formData,
						cache:false,
						contentType: false,
						processData: false,
