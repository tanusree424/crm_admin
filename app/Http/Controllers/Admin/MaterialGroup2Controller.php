...
class MaterialGroup2Controller extends Controller
{
    public function index()
    {

        $materialgroup1code = MaterialGroup2::latest()->get();
        ...
        return view('admin.materialgroup2.index',compact('basic','title','footertext'))->with($data)->with('i', (request()->input('page', 1) - 1) * 5);
    }
    ...
    public function store(Request $request)
    {
        ...
        if($validator->passes()){
            ...
            $project = MaterialGroup2::updateOrCreate(['id' => $testiId], $testi);
    ...
    public function destroy($id)
    {
      ...
      $testimonial = MaterialGroup2::find($id);
      $testimonial->delete();

      return response()->json(['success'=> lang('The material group 2 successfully deleted.', 'alerts')]);
    }

    public function materialmassdestroy(Request $request){
        $student_id_array = $request->input('id');

        $projects = MaterialGroup2::whereIn('id', $student_id_array)->get();
    ...
    public function projectassignee(Request $r)
    {
        ...
        $projects = MaterialGroup2::find($r->projected);
        if(!empty($projects)){
        foreach ($projects as $project){
    ...
    public function projetimport(){
        ...
        return view('admin.materialgroup2.projectimport')->with($data);
    }
